...
async function main () {
  ...
  try {
    ...
    await store.saveAssignment({ ...args, noteId: 'z', expectedVersion: 0, milestoneId: doomed.id })
    await assert.rejects(store.deleteMilestone({ id: doomed.id, namespace: 'o/r', expectedVersion: 99, actor: 'admin' }), /changed/)
    await assert.rejects(store.deleteMilestone({ id: doomed.id, namespace: 'other/r', expectedVersion: doomed.version, actor: 'admin' }), /changed/)
    assert.equal(await store.deleteMilestone({ id: doomed.id, namespace: 'o/r', expectedVersion: doomed.version, actor: 'admin' }), 1)
    assert.equal(await store.getMilestone(doomed.id, 'o/r'), null)
    const released = (await store.read({ namespaces: ['o/r'] })).assignments.find(x => x.noteId === 'z')
    assert.equal(released.milestoneId, null)
    assert.equal(released.version, 2)
    const releaseEvents = await pool.query("SELECT value FROM spec_board_planning_events WHERE note_id='z' AND action='milestone' ORDER BY id")
    assert.equal(releaseEvents.rows.at(-1).value.deletedMilestone, String(doomed.id))
